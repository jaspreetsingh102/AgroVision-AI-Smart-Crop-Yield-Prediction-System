...
async function analyzeSoil() {
  ...
  fileInput.onchange = async (e) => {
    ...
    resultDiv.innerHTML = `<div style="color: var(--farm-gold);"><i class="fas fa-spinner fa-spin"></i> Uploading & Analyzing...</div>`;
    ...
    try {
      ...
      if (res.ok) {
        let resultsHTML = `<div style="color: var(--farm-emerald); font-weight: 600; margin-bottom:1rem;">✅ Analysis Complete (${data.results.length} Models)</div>`;
        
        data.results.forEach(result => {
          if (result.error) {
            resultsHTML += `
              <div style="border-bottom: 1px solid var(--surface-hover); padding-bottom: 1rem; margin-bottom: 1rem; border-left: 4px solid #ef4444; padding-left: 10px;">
                <div style="font-size: 0.8rem; color: var(--text-muted);">Model: ${result.model}</div>
                <div style="font-weight: 600; color: #ef4444;">❌ Error: Model failed to process image.</div>
                <p style="font-size: 0.75rem; color: #f87171; margin-top: 0.25rem;">${result.error}</p>
              </div>`;
          } else {
            resultsHTML += `
              <div style="border-bottom: 1px solid var(--surface-hover); padding-bottom: 1rem; margin-bottom: 1rem; border-left: 4px solid var(--farm-emerald); padding-left: 10px;">
                <div style="font-size: 0.8rem; color: var(--text-muted);">Model: ${result.model}</div>
                <div style="font-weight: 600; color: var(--farm-gold);">Prediction: ${result.prediction}</div>
                ${result.details ? `<p style="font-size: 0.9rem; margin: 0.5rem 0;">pH: ${result.details.pH} | Organic: ${result.details.organic_matter}</p><p style="font-size: 0.85rem; color: var(--farm-emerald);">${result.details.recommendation}</p>` : ''}
              </div>`;
          }
        });
