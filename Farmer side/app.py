...
STYLES_DIR = os.path.join(BASE_DIR, F_DIR, 'styles')
...
# Mapping for Pest/Disease data
PEST_DATA_MAP = {
    "Healthy": {
        "status": "Clear",
        "recommendation": "Maintain current irrigation and nutrition schedule."
    },
    "Aphids": {
        "status": "Infestation Detected",
        "recommendation": "Apply Neem oil spray or organic insecticidal soap."
    },
    "Rust": {
        "status": "Fungal Infection",
        "recommendation": "Remove infected leaves and apply sulfur-based fungicide."
    }
}

# Mapping for Vegetation Status data
VEGETATION_DATA_MAP = {
    "Healthy": {
        "status": "Optimal",
        "recommendation": "Vegetation index is high. Maintain current schedule."
    },
    "Stressed": {
        "status": "Needs Attention",
        "recommendation": "Check for drought or nutrient deficiency."
    },
    "Sparse": {
        "status": "Warning",
        "recommendation": "Growth is stunted. Consider fertilization."
    }
}
...
"Experience": int(float(data.get('experience') or 0)),
"Farm_Area": int(float(data.get('farm_size') or 0)),
    ...
    try:
        ...
        for model_name, model_obj in MODELS.items():
            try:
                ...
                # Get label and strip "Class " prefix if present (e.g., "Class 6" becomes "6")
                classes = getattr(model_obj, 'classes_', DEFAULT_CLASS_NAMES)
                raw_label = str(classes[prediction_idx]) if prediction_idx < len(classes) else str(prediction_idx)
                detected_label = raw_label.replace("Class ", "").strip()
                
                # Combine Soil and Pest maps for detailed lookups
                merged_map = {**SOIL_DATA_MAP, **PEST_DATA_MAP}
                details = merged_map.get(detected_label, None)
...
@app.route('/analyze-pest', methods=['POST'])
def analyze_pest():
    # Check for the pest model file
    model_name = next((m for m in MODELS if 'pest_model' in m), None)
    
    if not model_name:
        return jsonify({"status": "error", "message": "Pest model file not found in ML folder"}), 500
    ...
    try:
        ...
        avg_color_features = np.mean(img_np, axis=(0, 1))
        ...
        prediction_val = prediction[0]
        if hasattr(prediction_val, "__len__"):
            prediction_idx = int(np.argmax(prediction_val))
        else:
            prediction_idx = int(prediction_val)
        ...
        # Load label_encoders from MODELS to decode the index
        encoders_name = next((m for m in MODELS if 'label_encoders' in m or 'lable_encoders' in m), None)
        if encoders_name:
            label_encoders = MODELS[encoders_name]
            if isinstance(label_encoders, dict) and 'pest_detected' in label_encoders:
                try:
                    detected_label = label_encoders['pest_detected'].inverse_transform([prediction_idx])[0]
                except Exception as e:
                    print("Pest label encoder failed:", e)

        # Fallback if label encoder not present or failed
        if detected_label == "Unknown":
            classes = getattr(model_obj, 'classes_', DEFAULT_CLASS_NAMES)
            raw_label = str(classes[prediction_idx]) if prediction_idx < len(classes) else str(prediction_idx)
            detected_label = raw_label.replace("Class ", "").strip()
        
        details = PEST_DATA_MAP.get(detected_label, {
            "status": "Unknown", 
            "recommendation": "Consult with local agricultural expert."
        })
...
@app.route('/analyze-vegetation', methods=['POST'])
def analyze_vegetation():
    # Use vegetation_model explicitly.
    model_name = next((m for m in MODELS if 'vegetation_model' in m), None)
    
    if not model_name:
        return jsonify({"status": "error", "message": "Vegetation model file not found in ML folder"}), 500
    ...
    try:
        ...
        # Load label_encoders from MODELS
        encoders_name = next((m for m in MODELS if 'label_encoders' in m or 'lable_encoders' in m), None)
        if encoders_name:
            label_encoders = MODELS[encoders_name]
            if isinstance(label_encoders, dict) and 'vegetation_health' in label_encoders:
                try:
                    detected_label = label_encoders['vegetation_health'].inverse_transform([prediction_idx])[0]
                except Exception as e:
                    print("Label encoder failed:", e)
        ...
        details = VEGETATION_DATA_MAP.get(detected_label, {
            "status": "Unknown", 
            "recommendation": "Consult with local agricultural expert."
        })

        import random
        # Mock environment data associated with the image analysis
        env_metrics = {
            "temperature": f"{random.randint(24, 38)}°C",
...
def serve_any(filename):
    ...
    for folder in [PAGES_DIR, SCRIPTS_DIR, ASSETS_DIR, STYLES_DIR]:
